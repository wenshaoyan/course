<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table  :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row >
      ...
      <el-table-column align="center" label="版本名称" width="95">
        <template scope="scope">
      ...
      <el-table-column align="center" label="下载地址">
        <template scope="scope">
          <span>{{scope.row.download_url}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="更新描述" >
        <template scope="scope">
...
<script>
  import { clientIdVersionQuery, clientQueryPattern, versionQuery, versionUpdate, versionInsert } from '@/api/client'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        this.getExecFunction().then(data => {
          this.list = data.list
      ...
      getExecFunction() {
        ...
        query = Object.assign(query, this.versionQuery)

        if (this.versionQuery.client_id === undefined || this.versionQuery.client_id === null || this.versionQuery.client_id === '') {
          f = versionQuery
        } else {
          args.push(this.versionQuery.client_id)
          f = clientIdVersionQuery
        }
        args.push(query)
        return f(...args)
      },
